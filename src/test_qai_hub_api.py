    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print_info("這可能是正常的，因為你的 MacBook 不是 Snapdragon 設備")
                return True  # 仍然返回 True，因為基本功能正常
        except Exception as e:
...
def diagnose_network():
    ...
    try:
        ...
        with socket.create_connection(("api.qai-hub.qualcomm.com", 443)) as sock:
            with context.wrap_socket(sock, server_hostname="api.qai-hub.qualcomm.com") as ssock:
                print_success(f"TLS 連接成功: {ssock.version()}")
    except Exception as e:
        print_error(f"TLS 連接失敗: {e}")
...
def main():
    ...
    print("\\n📋 如需連接 QAI Hub API:")
    print("   1. 運行 fix_qai_hub_macos.sh 修復工具")
...
def main():
    """主函數"""
    print_header("QAI Hub API 認證診斷工具")
