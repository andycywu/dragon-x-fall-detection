"""Shim to re-export the canonical ElderlyBehaviorPredictor implementation.

This file keeps backward compatibility for imports that referenced
`src/infer_demo_Mac/elderly_behavior_predictor.py` by redirecting to the
single canonical implementation at `src/elderly_behavior_predictor.py`.

Do not modify this shim — update the top-level implementation instead.
"""
import os
import sys
...
try:
    from elderly_behavior_predictor import ElderlyBehaviorPredictor  # canonical
except Exception:
    # last-resort: try relative import (very unlikely)
    try:
        from src.elderly_behavior_predictor import ElderlyBehaviorPredictor
    except Exception:
        ElderlyBehaviorPredictor = None

__all__ = ['ElderlyBehaviorPredictor']
...
class ElderlyBehaviorPredictor:
    ...
    def identify_user(self, frame: Any) -> Optional[str]:
        """
    ...
    def update_posture_history(self, user_id: str, frame: Any) -> bool:
        """
        更新用戶姿態歷史記錄
    ...
    def process_user_interaction(self, user_id: str, frame: Any) -> Dict[str, Any]:
        """
