...
class QAIHubOptimizer:
    ...
    def __init__(self):
        ...
        # Debug: 打印 client.ini 路徑與內容
        import os
        client_ini_path = os.path.expanduser('~/.qai_hub/client.ini')
        print(f"[DEBUG] QAI Hub client.ini 路徑: {client_ini_path}")
        if os.path.exists(client_ini_path):
            with open(client_ini_path, 'r') as f:
                print("[DEBUG] client.ini 內容:\n" + f.read())
        else:
            print("[DEBUG] client.ini 不存在！")
        self.setup_qai_hub()
