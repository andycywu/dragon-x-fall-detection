...
def test_pose_visualization():
    ...
    if not Path(test_image_path).exists():
        ...
        while True:
            ...
            # 切換檢測方法
            detector.switch_detection_method(current_method)
            ...
            if success and landmarks:
                # 繪製姿態骨架
                frame = draw_pose_skeleton(frame, landmarks, current_method)
                
                # 顯示檢測信息
                info_text = f"Method: {current_method} | Keypoints: {len(landmarks)}"
                cv2.putText(frame, info_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                
                status_text = f"Info: {info}"
        ...
        for i, method in enumerate(methods):
            ...
            success, landmarks, info = detector.detect_pose(image.copy())
            
            if success and landmarks:
                print(f"  ✅ 檢測成功，關鍵點數量: {len(landmarks)}")
                
                # 繪製姿態骨架
                vis_image = draw_pose_skeleton(image.copy(), landmarks, method)
                
                # 添加標題
                title = f"{method} - {len(landmarks)} keypoints"
