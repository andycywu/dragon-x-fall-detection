...
class CompletelyFixedHackathonDetector:
    ...
    def _generate_center_body_model(self, width=640, height=480):
        """生成圖像中心的人體模型"""
        try:
            landmarks = []
            center_x = width // 2
    ...
    def _generate_adaptive_landmarks(self, boxes: List, confidences: List) -> List[Tuple[float, float]]:
        """基於多個檢測框生成自適應關鍵點"""
        if not boxes:
            return self._generate_center_body_model()
    ...
    def _apply_motion_prediction(self, current_landmarks: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
        """基於運動預測優化關鍵點位置"""
        if not hasattr(self, '_previous_landmarks') or not self._previous_landmarks:
            self._previous_landmarks = current_landmarks
            return current_landmarks
        
        # 計算運動向量
        predicted_landmarks = []
        for i, (curr_point, prev_point) in enumerate(zip(current_landmarks, self._previous_landmarks)):
            if curr_point and prev_point:
                ...
                # 平滑處理
                smooth_factor = 0.7
                predicted_x = curr_point[0] + dx * smooth_factor * 0.1
                predicted_y = curr_point[1] + dy * smooth_factor * 0.1
                
                predicted_landmarks.append((predicted_x, predicted_y))
            else:
                predicted_landmarks.append(curr_point if curr_point else (320.0, 240.0))
        
        self._previous_landmarks = predicted_landmarks
        return predicted_landmarks
    
    def _calculate_enhanced_confidence(self, landmarks: List[Tuple[float, float]], 
                                     boxes: List = None, confidences: List = None) -> float:
        """計算增強的檢測可信度"""
        if not landmarks:
            return 0.0
        ...
        if boxes and confidences:
            box_bonus = min(len(boxes) * 0.1, 0.3)  # 最多30%獎勵
            avg_conf = sum(confidences) / len(confidences) if confidences else 0.5
            conf_bonus = avg_conf * 0.2  # 最多20%獎勵
            base_confidence += box_bonus + conf_bonus
        
        # 基於關鍵點分佈的評估
        if len(landmarks) >= 10:
            # 檢查關鍵點是否有合理的分佈
            x_coords = [p[0] for p in landmarks if p and len(p) >= 2]
            y_coords = [p[1] for p in landmarks if p and len(p) >= 2]
            ...
            if x_coords and y_coords:
                ...
                # 良好的分佈應該有一定的範圍
                if x_range > 50 and y_range > 100:  # 合理的人體尺寸
                    base_confidence += 0.2
