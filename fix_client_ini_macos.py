#!/usr/bin/env python3
"""
QAI Hub client.ini 修復工具 - macOS 專用版本
修復 QAI Hub 認證和連接問題
"""
...
def fix_client_ini_macos():
    """專門為 macOS 修復 client.ini 文件"""
    print_header("QAI Hub client.ini macOS 修復工具")
    
    # 檢查是否是 macOS 系統
    if platform.system() != "Darwin":
        print_warning(f"這個工具是為 macOS 設計的，當前系統是 {platform.system()}")
    
    # 記錄系統信息
    print_info(f"macOS 版本: {platform.mac_ver()[0]}")
    ...
    try:
        ...
        try:
            ...
            print_success("目錄權限檢查通過")
        except Exception as e:
            print_error(f"目錄權限問題: {e}")
            # 嘗試修復權限
            try:
                subprocess.run(['chmod', '700', str(config_dir)], check=True)
                print_success("已嘗試修復目錄權限")
            except Exception as e2:
                print_error(f"修復權限失敗: {e2}")
    except Exception as e:
    ...
    if created and config_file.exists():
        ...
        # 設置文件權限
        try:
            os.chmod(config_file, 0o600)  # 只有用戶可讀寫
            print_success("文件權限已設置")
        except Exception as e:
            print_warning(f"設置文件權限失敗: {e}")
    ...
    for domain in domains:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # 嘗試使用系統 DNS 解析
                try:
                    dns_result = subprocess.run(['host', domain, '8.8.8.8'], 
                                               capture_output=True, text=True, timeout=5)
                    if dns_result.returncode == 0:
                        print_info(f"使用公共 DNS 可以解析 {domain}")
                        print_info("這表明可能是 DNS 問題，建議嘗試更改 DNS 設置")
                    else:
                        print_warning(f"即使使用公共 DNS 也無法解析 {domain}")
                except Exception:
                    pass
        except Exception as e:
...
def main():
    """主函數"""
    print_header("QAI Hub macOS 診斷與修復工具")
    
    # 步驟 1: 修復 client.ini
    success = fix_client_ini_macos()
    ...
    if success and network_ok and qai_hub_ok:
        ...
        if not success:
            print_info("1. 手動編輯 ~/.qai_hub/client.ini 文件")
            print_info("   格式應為:")
            print_info("   [default]")
    ...
    print_info("   cp ~/.qai_hub/client.ini.uppercase ~/.qai_hub/client.ini")
    print_info("2. 確保設置了環境變量:")
    print_info("   export QAI_HUB_API_TOKEN=pcu8nz63e4j3nzqgy7tjzvr2dmpc01cocltahr0d")
    ...
    print_info("4. 確認 MacOS 安全設置允許應用程序連接網絡")
    print_info("5. 如使用公司/學校網絡，請向網絡管理員確認是否限制了對 qualcomm.com 的訪問")
