...
class ElderlyBehaviorPredictor:
    ...
    def register_user(self, user_id: str, name: str, image_path: str, 
        ...
        """
        try:
            logger.info(f"🔄 開始註冊用戶: {name} (ID: {user_id})")
            logger.info(f"📷 照片路徑: {image_path}")
            
            # 檢查文件是否存在
            if not os.path.exists(image_path):
                logger.error(f"❌ 照片文件不存在: {image_path}")
                return False
            
            # 載入圖片
            image = face_recognition.load_image_file(image_path)
            logger.info(f"✅ 成功載入圖片，尺寸: {image.shape}")
            ...
            for model in models:
                try:
                    logger.info(f"🔍 使用 {model.upper()} 模型檢測人臉...")
                    face_locations = face_recognition.face_locations(image, model=model)
                    if face_locations:
                        logger.info(f"✅ {model.upper()} 模型檢測到 {len(face_locations)} 個人臉")
                        break
                    else:
                        logger.warning(f"⚠️ {model.upper()} 模型未檢測到人臉")
                except Exception as e:
                    logger.warning(f"⚠️ {model.upper()} 模型檢測失敗: {e}")
                    continue
            
            if not face_locations:
                logger.error(f"❌ 所有模型都未能檢測到人臉: {image_path}")
                logger.error(f"💡 建議: 請確保照片清晰，人臉完整可見，且光線充足")
                return False
            ...
            if not face_encodings:
                logger.error(f"❌ 無法提取人臉編碼: {image_path}")
                return False
            
            # 使用第一個檢測到的人臉
            face_encoding = face_encodings[0]
            self.known_faces[user_id] = face_encoding
            logger.info(f"✅ 成功提取人臉編碼，向量維度: {len(face_encoding)}")
            ...
            logger.info(f"💾 人臉編碼已保存到: {self.face_encodings_path}")
            ...
            logger.info(f"📊 當前已註冊用戶數量: {len(self.known_faces)}")
            return True
    ...
    def generate_behavioral_summary(self, user_id: str) -> str:
        """生成行為分析摘要"""
        try:
            # 使用正確的數據庫連接屬性
            cursor = self.db_conn.cursor()
            cursor.execute("""
                SELECT * FROM pose_analysis 
                WHERE user_id = ? 
            ...
            # 計算平均指標
            avg_balance = sum(row[2] for row in recent_data) / len(recent_data)
            avg_stability = sum(row[3] for row in recent_data) / len(recent_data)
            avg_risk = sum(row[5] for row in recent_data) / len(recent_data)
            ...
            {"⚠️ 建議多加注意身體平衡" if avg_risk > 0.6 else "✅ 整體狀況良好"}
            """
    ...
    def ask_user_checkin_question(self) -> str:
        """詢問用戶健康狀況"""
        try:
            questions = [
                "您今天感覺如何？",
