...
class QAIHubUnifiedDetector:
    ...
    def detect_faces(self, image: np.ndarray) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"QAI Hub人臉檢測結果類型: {type(result)}")
            
            faces = []
            if result is not None:
                try:
                    # 嘗試多種解析方式
                    logger.info(f"檢測結果: {result}")
                    
                    # 如果是list類型
                    if isinstance(result, list) and len(result) > 0:
                        for i, face_data in enumerate(result):
                            logger.info(f"處理第{i}個人臉數據: {type(face_data)}")
                            if hasattr(face_data, 'landmark') and face_data.landmark:
                                face_dict = self._extract_face_from_landmarks(face_data.landmark, image)
                                if face_dict:
                                    faces.append(face_dict)
                    # 如果直接是landmark對象
                    elif hasattr(result, 'landmark') and result.landmark:
                        logger.info("檢測到單個人臉landmark")
                        face_dict = self._extract_face_from_landmarks(result.landmark, image)
                        if face_dict:
                            faces.append(face_dict)
                    # 如果是字典或其他格式
                    else:
                        logger.info(f"未識別的結果格式: {type(result)}")
                        
                except Exception as parse_error:
                    logger.error(f"解析人臉檢測結果失敗: {parse_error}")
            
            logger.info(f"最終檢測到 {len(faces)} 個人臉")
            return faces
    ...
    def _extract_face_from_landmarks(self, landmarks, image):
        """從landmark提取人臉信息"""
        try:
            h, w = image.shape[:2]
            ...
            logger.error(f"提取人臉landmark失敗: {e}")
        return None
