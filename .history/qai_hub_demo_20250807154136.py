...
def demo_configuration():
    ...
    try:
        ...
        print(f"📱 API Token 狀態: {'已設置' if config.qai_hub['api_token'] != 'your_api_token_here' else '未設置'}")
        print(f"🚀 硬件加速: {'啟用' if config.qai_hub['enable_acceleration'] else '禁用'}")
        print(f"⚡ 優化級別: {config.qai_hub['optimization_level']}")
        print(f"🎯 設備類型: {config.qai_hub['device_type']}")
        print()
