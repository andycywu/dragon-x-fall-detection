...
class RealQAIHubONNXDetector:
    ...
    def _verify_qai_hub_connection(self):
        ...
        try:
            ...
            # 選擇目標設備 - 優先選擇Snapdragon X (Dragon X)
            # 1. 優先選擇Snapdragon X Elite系列
            snapdragon_x_devices = [d for d in devices if 'Snapdragon X' in d.name or 'X Elite' in d.name or 'Dragon X' in d.name]
            
            # 2. 其次選擇其他Snapdragon設備
            other_snapdragon_devices = [d for d in devices if 'Snapdragon' in d.name and d not in snapdragon_x_devices]
            
            # 3. 最後選擇Samsung設備作為後備
            samsung_devices = [d for d in devices if 'Samsung' in d.name]
            
            if snapdragon_x_devices:
                self.target_device = snapdragon_x_devices[0]
                logger.info(f"🐉 優先選擇Dragon X設備: {self.target_device.name}")
                logger.info(f"🎯 黑客松主打設備已選定!")
            elif other_snapdragon_devices:
                self.target_device = other_snapdragon_devices[0]
                logger.info(f"🔥 選擇Snapdragon設備: {self.target_device.name}")
            elif samsung_devices:
                self.target_device = samsung_devices[0]
                logger.info(f"📱 後備選擇Samsung設備: {self.target_device.name}")
            else:
                # 使用第一個可用設備
            ...
            if self.target_device:
                logger.info(f"📊 目標設備資訊:")
                logger.info(f"   設備名稱: {self.target_device.name}")
                logger.info(f"   設備類型: {'🐉 Dragon X' if 'X' in self.target_device.name else '📱 移動設備'}")
        ...
        try:
            ...
            if self.enable_acceleration and self.target_device:
                ...
                try:
                    ...
                    logger.error(f"❌ 人臉檢測編譯Job提交失敗: {compile_error}")
    ...
    def _load_pose_estimation_model(self):
        ...
        try:
            ...
            self.qai_hub_models['pose'] = pose_detector
            ...
            if self.enable_acceleration and self.target_device:
                ...
                try:
                    ...
                    logger.error(f"❌ 姿態估計編譯Job提交失敗: {compile_error}")
    ...
    def _load_hand_detection_model(self):
        ...
        try:
            ...
            self.qai_hub_models['hand'] = hand_detector
            ...
            if self.enable_acceleration and self.target_device:
                ...
                try:
                    ...
                    logger.error(f"❌ 手部檢測編譯Job提交失敗: {compile_error}")
    ...
    def export_to_onnx(self, model_name: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                # 方法1: 先轉換為TorchScript再導出ONNX
                torchscript_model = model.convert_to_torchscript(
                    example_inputs=sample_input
                )
                
                # 使用QAI Hub導出ONNX
                onnx_model = hub.get_onnx_model(
                ...
                logger.warning(f"⚠️ TorchScript方法失敗: {e1}")
                ...
                try:
                    ...
                    logger.info(f"✅ ONNX模型已保存 (直接方法): {onnx_path}")
                    return onnx_path
                    
                except Exception as e2:
                    logger.error(f"❌ 直接導出也失敗: {e2}")
                    return None
    ...
    def _preprocess_image(self, image: np.ndarray, model_name: str) -> np.ndarray:
        """預處理圖像"""
        # 統一使用256x256尺寸避免形狀不匹配
        if model_name == 'face':
            target_size = (256, 256)  # 改為256x256
        elif model_name == 'pose':
            target_size = (256, 256)
        elif model_name == 'hand':
            target_size = (224, 224)
        else:
            target_size = (256, 256)  # 默認256x256
